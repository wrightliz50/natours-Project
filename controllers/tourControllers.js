...
//refactor code to keep simple, use function for callback
...
//     results: tours.length, //nice to see how many tours there are that are sent to make sure it matches up
//     data: {
//       tours: tours,
//     },
//   });
...
//populate reviews with virtual query
...
//Delete tour replaced by factory.deleteOne
...
exports.getTourStats = catchAsync(async (req, res, next) => {
  const stats = await Tour.aggregate([
    ...
    {
      $group: {
        _id: null, //'$difficulty', //'$ratingsAverage',
        numTours: { $sum: 1 },
...
// to make a route for distance from tour tours-within/300/center/34.1,-118.1/unit/mi
exports.getToursWithin = catchAsync(async (req, res, next) => {
  const { distance, latlong, unit } = req.params;
  const [lat, long] = latlong.split(',');

  //convert earth radius to radians
  const radius = unit === 'mi' ? distance / 3963.2 : distance / 6378.1;

  if (!lat || !long) {
    next(new AppError('Please provide lat/long', 400));
  }
